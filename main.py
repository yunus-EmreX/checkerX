...
class Netflixer:
    ...
    def ui(self):
        ...
        # Ekrana yazdırma
        print(faded.replace('shellbee', 'ChatGPT'))
    ...
    def updateTitle(self):
        while True:
            ...
            print(f'[NETFLIXER v3] - Hits: {self.hits} | Bad: {self.bad} | Retries: {self.retries} | CPM: {self.cpm} | Threads: {threading.active_count() - 2} | Time elapsed: {elapsed}')
            time.sleep(0.4)
    ...
    def getCombos(self):
        try:
            ...
            print(f'[{Fore.LIGHTRED_EX}!{Fore.RESET}] Failed to open combo file')
            os.system('pause >nul')
            quit()
    ...
    def checker(self, email, password):
        try:     
            ...
            if 'Sorry, we can\'t find an account with this email address. Please try again or' in request.text or 'Incorrect password' in request.text:
                self.lock.acquire()
